object SolutionHW1 {
  def mergeSort[T](vector: Vector[T])(implicit ord: Ordering[T]): Vector[T] = {
    def merge(left: Vector[T], right: Vector[T], acc: Vector[T] = Vector.empty): Vector[T] = {
  ...
  def mergeSortDistinct[T](vector: Vector[T])(implicit ord: Ordering[T]): Vector[T] = {
    def merge(left: Vector[T], right: Vector[T], last: Option[T] = None, acc: Vector[T] = Vector.empty): Vector[T] = {
      (left.headOption, right.headOption) match {
        case (None, None) => acc
        case (None, Some(r)) =>
